...
struct GearTeethSet_t
  {
      String    name;
      uint8_t   Gear[11];
  };

GearTeethSet_t frontGearTeethSets[] = {
                      {"34/50T", {34, 50}},         // 11 speed 34/50T
                      {"36/52T", {36, 52}},         // 11 speed 36/52T
                      {"39/53T", {39, 53}}          // 11 speed 39/53T
                  };


// https://neweracycle.com/shimano-ultegra-r8000-11-speed-road-cassette

GearTeethSet_t rearGearTeethSets[] = {
                      {"11-32T", {11, 12, 13, 14, 16, 18, 20, 22, 25, 28, 32}},       // 11 speed 11-32T
                      {"11-30T", {11, 12, 13, 14, 15, 17, 19, 21, 24, 27, 30}},       // 11 speed 11-30T
                      {"11-28T", {11, 12, 13, 14, 15, 17, 19, 21, 23, 25, 28}},       // 11 speed 11-28T
                      {"11-25T", {11, 12, 13, 14, 15, 16, 17, 19, 21, 23, 25}},       // 11 speed 11-25T
                      {"12-25T", {12, 13, 14, 15, 16, 17, 18, 19, 21, 23, 25}},       // 11 speed 12-25T
                      {"12-28T", {12, 13, 14, 15, 16, 17, 19, 21, 23, 25, 28}}        // 11 speed 12-28T
                  };
...
/*** Gearing ***/
static uint8_t frontGearTeethSetsCount  = sizeof(frontGearTeethSets) / sizeof(frontGearTeethSets[0]);
static uint8_t rearGearTeethSetsCount   = sizeof(rearGearTeethSets) / sizeof(rearGearTeethSets[0]);


static int8_t frontGearTeethSet;                        // gear set to use from the gearing data array (frontGearTeethSets - maintained in preferences
static int8_t rearGearTeethSet;                         // gear set to use from the gearing data array (rearGearTeethSets) - maintained in preferences

static uint8_t frontGear                = 1;            // front gear, updated by bluetooth messages from trainer
static uint8_t rearGear                 = 3;            // rear gear, updated by bluetooth messages from trainer
static uint8_t nofFrontGears            = 2;            // number of front gears on trainer, updated by bluetooth messages from trainer
static uint8_t nofRearGears             = 12;           // number of rear gears on trainer, updated by bluetooth messages from trainer
static uint8_t frontGearTeeth           = 50;           // current front gear determined by looking up the frontGear from bluetooth in the configured gear set (default to 50)
static uint8_t rearGearTeeth            = 23;           // current rear gear determined by looking up the frontGear from bluetooth in the configured gear set (default to 34)
...
{  
  ...
  { 
    ...
    frontGearTeeth   = frontGearTeethSets[frontGearTeethSet - 1].Gear[frontGear - 1];
    //frontGearTeeth  = frontGearTeethSets[frontGearTeethSet].Gear[frontGearIndex - 1];

    rearGear   = p_data[3] + 1;
    rearGearTeeth   = rearGearTeethSets[rearGearTeethSet - 1].Gear[11 - rearGear];
...
static void updateGear(void) 
{
  ...
  String gearRatio = String(frontGearTeeth) + ":" + String(rearGearTeeth);
  ...
  if(gearingLimitReached == true) 
    ...
    img.setTextColor(TFT_SKYBLUE, TFT_BLACK);
  img.drawString(gearRatio, (RESOLUTION_X/2) - 70, 20, 7);  
...
*/
  ...
  img.drawString("F:" + frontGearTeethSets[frontGearTeethSet - 1].name + "  R:" + rearGearTeethSets[rearGearTeethSet - 1].name, RESOLUTION_X - 140, RESOLUTION_Y - 16, 2);
  ...
  else 
  {
    ...
    img.setTextColor(TFT_SKYBLUE, TFT_BLACK);
    img.drawString("Stephen's Wahoo Kickr Display", 0, 0, 4);
  ...
  if((topButton != topButtonState) || (bottomButton != bottomButtonState))
  {
    ...
    if(topButton != topButtonState) 
    {
      ...
      if(topButton != topButtonState)
      {
        ...
        {
          frontGearTeethSet++;
          if(frontGearTeethSet > frontGearTeethSetsCount)
            frontGearTeethSet = 1;
          preferences.putInt("frontSet", frontGearTeethSet);
          updateDisplay();
        }
      }
    }
    if(bottomButton != bottomButtonState) 
    {
      ...
      if(bottomButton != bottomButtonState)
      {
        ...
        {
          rearGearTeethSet++;
          if(rearGearTeethSet > rearGearTeethSetsCount)
            rearGearTeethSet = 1;
          preferences.putInt("rearSet", rearGearTeethSet);
          updateDisplay();
        }
      }
    }
...
void setup(void) 
{
  // retrieve perferences
  preferences.begin("config", false);
  frontGearTeethSet = preferences.getInt("frontSet", 1);
  rearGearTeethSet  = preferences.getInt("rearSet", 1);
